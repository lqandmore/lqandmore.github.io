...
使用[vite](https://cn.vitejs.dev/guide/)创建项目
...
这里我们可以使用自动配置命令：(windows上使用 git bush，)

```shell
npx husky-init && npm install      // npx husky-init '&&' npm install  for win
```
...
[Element Plus](https://element-plus.gitee.io)，一套为开发者、设计师和产品经理准备的基于 Vue 3.0 的桌面端组件库：
